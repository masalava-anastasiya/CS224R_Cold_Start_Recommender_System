"""Evaluate and compare GreedyCFBaseline against RandomBaseline.

Metrics
-------
Avg reward / step    — mean rating per recommended item (order-invariant when
                       all T candidates are exhausted, but reported for
                       completeness).
Avg reward @ step 1  — quality of the very first recommendation.
Avg reward @ k       — mean rating among the first k recommendations;
                       measures early-precision without the log-discount of
                       NDCG.
NDCG@k               — primary ranking metric; rewards surfacing high-rated
                       items early.  Computed against the ideal (descending)
                       ordering of actual ratings.
Hit@k                — fraction of episodes where >= 1 of the first k
                       recommendations has raw rating >= 4.

Note on cumulative reward: because each episode uses exactly T candidates over
T steps, every policy selects all items and accumulates identical total reward
per user.  It is reported for reference but does not distinguish policies.
"""
...
# ---------------------------------------------------------------------------
# Episode runner and metric aggregation
# ---------------------------------------------------------------------------

def run_episodes(policy, env: ColdStartEnv, cold_users: List[int], label: str) -> List[List[float]]:
    #Run one episode per cold user; return list of per-episode reward lists
...
def main() -> None:
    ...
    # Build policies
    print(f"\nFitting GreedyCFBaseline (k={K_FACTORS}, λ={REG})...")
    t0 = time.time()
    ...
    def row(label, gcf_key, rnd_key=None, fmt=".4f", show_std=True):
        rnd_key = rnd_key or gcf_key
        std_key = gcf_key.replace("avg_", "std_").replace("ndcg_", "std_ndcg_")
        rnd_std_key = rnd_key.replace("avg_", "std_").replace("ndcg_", "std_ndcg_")
        if show_std and std_key in gcf_m:
            gcf_str = f"{gcf_m[gcf_key]:{fmt}} ± {gcf_m[std_key]:{fmt}}"
            rnd_str = f"{rnd_m[rnd_key]:{fmt}} ± {rnd_m[rnd_std_key]:{fmt}}"
        else:
            gcf_str = f"{gcf_m[gcf_key]:{fmt}}"
            rnd_str = f"{rnd_m[rnd_key]:{fmt}}"
        print(f"  {label:<26} {gcf_str:<22} {rnd_str}")

    print("\n" + "=" * 70)
    print("  GreedyCFBaseline vs. RandomBaseline")
    print("=" * 70)
    print(f"  Cold users: {len(cold_users)}  |  T: {T}  |  k (MF): {K_FACTORS}  |  λ: {REG}  |  eval@{K_EVAL}")
    print("=" * 70)
    print(f"  {'Metric':<26} {'Greedy CF':<22} Random")
    print("  " + "-" * 66)
    row(f"Avg reward @ step 1",       "avg_reward_step1")
