"""Niche-taste cold-user case study (prior mismatch vs exploration)."""
...
def quartile_analysis(
    ...
    for method, all_rewards in rewards_by_method.items():
        ...
        late = [
            float(np.mean(r[early_steps:])) if len(r) > early_steps else 0.0
...
def print_quartile_table(analysis: Dict, method_order: List[str]) -> None:
    ...
    print("\nquartile analysis (avg cumulative reward):")
    for method in method_order:
        d = analysis["per_method"][method]
        parts = [
            f"{qlabels[q]}={d['per_quartile_mean'][q]:.3f}±{d['per_quartile_std'][q]:.2f}"
            for q in range(4)
        ]
...
def plot_quartile_analysis(
    ...
    ax1.set_title("Cumulative reward by niche quartile")
    ax1.set_xticks(x)
    ...
    ax2.set_title("Exploration advantage by quartile")
    ax2.set_xticks(x)
...
def plot_relative_degradation(
    ...
    ax.set_ylabel("Relative reward (Q1 = 1.0)")
    ax.set_title("Relative degradation by niche quartile")
    ax.set_xticks(x)
...
def plot_recovery_curves(
    ...
    method_order: List[str],
    out_path: Path,
    early_window: Tuple[int, int] = (0, 5),
    late_window: Tuple[int, int] = (15, 20),
) -> None:
    ...
    for i, method in enumerate(method_order):
        ...
        avg_per_step = [
            float(np.mean([r[t] for r in rewards_q4]))
            for t in range(T)
        ]
        ax1.plot(
    ...
    ax1.axvspan(early_window[0] + 1, early_window[1], alpha=0.10, color="gray", zorder=0)
    ax1.axvspan(late_window[0] + 1, late_window[1], alpha=0.10, color="steelblue", zorder=0)
    ax1.text(
        (early_window[0] + early_window[1]) / 2 + 0.5, ylo + 0.05 * (yhi - ylo),
        f"early (1–{early_window[1]})", ha="center", va="bottom", fontsize=7, color="dimgray",
    )
    ax1.text(
        (late_window[0] + late_window[1]) / 2 + 0.5, ylo + 0.05 * (yhi - ylo),
        f"late ({late_window[0] + 1}–{late_window[1]})", ha="center", va="bottom",
        fontsize=7, color="steelblue",
    )

    ax1.set_xlabel("Step")
    ax1.set_ylabel("Avg reward")
    ax1.set_title(f"Per-step reward, Q4 users (n={n_q4})")
    ax1.legend(fontsize=8, ncol=2)
    ax1.grid(alpha=0.3)
    ...
    ax2.set_ylabel(
        f"Late minus early avg "
        f"(steps {late_window[0]+1}–{late_window[1]} vs {early_window[0]+1}–{early_window[1]})"
    )
    ax2.set_title("Recovery delta, Q4 users")
    ax2.grid(axis="y", alpha=0.3)
...
def main(args) -> None:
    ...
    if missing:
        print(f"Missing artifacts: {missing}. Run `python -m src.data.preprocess` first.")
        sys.exit(1)
    ...
    K_FACTORS = 50
    print(f"Fitting GreedyCFBaseline (k={K_FACTORS})...")
    t0 = time.time()
    greedy_cf = GreedyCFBaseline(
    ...
    if not ckpt_path.exists():
        print(f"RL² checkpoint not found at {ckpt_path}, skipping RL²")
        rl2 = None
    else:
    ...
    rewards_by_method: Dict[str, List[List[float]]] = {
        ...
        "Constrained": run_episodes(constrained, env, cold_users, "Constrained"),
    }
    if rl2 is not None:
        rewards_by_method["RL²"] = run_episodes(rl2, env, cold_users, "RL2")
    else:
...
def parse_args():
    ...
    p.add_argument("--n-vignettes", type=int, default=3)
