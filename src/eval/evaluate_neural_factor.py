"""Evaluate "true" Neural-Linear TS (learned features) vs all methods.

Trains the two-tower item tower on warm ratings, then runs Thompson
Sampling over the learned factors. Compares against NLTS (frozen
embeddings), Hybrid TS (SVD factors), Greedy CF, and Random on the full
cold set, under both protocols. Tests whether reward-trained features let
exploration finally match or beat greedy at full (70%) warm data.

Run from repo root:
    python -m src.eval.evaluate_neural_factor
"""
...
def main() -> None:
    ...
    # --- train the neural item tower (method #2) ---
    print("\n[Neural-Factor TS] training learned item factors...")
    t0 = time.time()
    ...
    hybrid = HybridNeuralLinearTS(ratings_by_user=ratings_by_user, warm_users=warm_users,
                                  n_items=n_items, k=K_FACTORS, lambda_prior=1.0, sigma_noise=1.0)
    neural_factor = NeuralFactorTS(Q=Q, item_means=item_means, mu_0=mu_0_nf,
                                   Lambda_0=Lambda_0_nf, sigma_noise=1.0)
    greedy = GreedyCFBaseline(ratings_by_user=ratings_by_user, warm_users=warm_users,
    ...
    all_results: Dict = {
        "experiment": {
            "method": "neural_factor_ts (true neural-linear, learned features)",
            "k_factors": K_FACTORS, "T": T, "k_eval": K_EVAL,
    ...
    for protocol, use_full in [("selection", True), ("ranking", False)]:
        ...
        def row(lab, key, fmt=".4f", show_std=True):
            ...
            print(f"  {lab:<18}" + "".join(f" {p:<18}" for p in parts))
