"""Dataset visualizations for the CS224R poster.

Produces one PNG per chart in results/figures/:
  01_rating_distribution.png
  02_ratings_per_user.png
  03_gender_split.png
  04_age_distribution.png
  05_occupation_distribution.png
  06_warm_cold_split.png
...
PALETTE = [CARDINAL, SKY, PALO_ALTO, POPPY, PURPLE,
...
def load_data():
    ...
    # keep only filtered users (those in ratings_by_user)
    surviving_raw_ids = set()
    for uid in ratings_by_user:
        pass
    # rebuild: map processed user_idx back to raw UserIDs via ratings_df
    surviving_raw_ids = set(
        ratings_df.groupby("UserID")["Rating"]
        .count()[lambda s: s >= 25].index
    )
...
# ---------------------------------------------------------------------------
# 2. Ratings per user (log-scale histogram)
# ---------------------------------------------------------------------------
...
def plot_ratings_per_user(ratings_per_user: list) -> None:
    ...
    ax.axvline(rpu.mean(),   color=CARDINAL, linewidth=2,
               linestyle="--", label=f"Mean = {rpu.mean():.0f}")
    ax.axvline(np.median(rpu), color=POPPY, linewidth=2,
               linestyle=":",  label=f"Median = {np.median(rpu):.0f}")
...
def plot_age(users_df: pd.DataFrame) -> None:
    age_labels = {1: "Under 18", 18: "18–24", 25: "25–34",
...
# ---------------------------------------------------------------------------
# 5. Occupation distribution (horizontal bar)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 6. Warm / cold split summary (stacked bar)
# ---------------------------------------------------------------------------
...
def plot_warm_cold(user_split: dict, ratings_by_user: dict) -> None:
    ...
    fig, ax = plt.subplots(figsize=(5, 4))
    b1 = ax.bar(x, warm_vals, width, label="Warm (train)",
                color=PALO_ALTO, edgecolor="white", linewidth=1.2)
    b2 = ax.bar(x, cold_vals, width, bottom=warm_vals,
                label="Cold (eval)", color=SANDSTONE, edgecolor="white", linewidth=1.2)
    ...
    ax.set_ylabel("Count (users) / Count (thousands of ratings)")
...
if __name__ == "__main__":
    ...
    print(f"  {len(ratings_by_user):,} users | {len(all_ratings):,} ratings")
    ...
    print(f"\nAll figures saved to {OUT_DIR.relative_to(_REPO_ROOT)}/")
